...
export const PlayerSeat: React.FC<PlayerSeatProps> = ({
  ...
  return (
    <div className={`relative ${isCurrentPlayer ? 'z-10' : ''}`}>
      ...
      `}>
        ...
        `}>
          <AvatarFallback className="bg-navy text-emerald font-bold">
            {getPlayerInitial()}
          </AvatarFallback>
        </Avatar>
        ...
        <div className="absolute -top-2 left-0 right-0 flex justify-center gap-1">
          {state.isDealer && (
            <span className="bg-yellow-500 text-black text-xs px-1 rounded">D</span>
          )}
          {state.isSmallBlind && (
            <span className="bg-blue-500 text-white text-xs px-1 rounded">SB</span>
          )}
          {state.isBigBlind && (
            <span className="bg-red-500 text-white text-xs px-1 rounded">BB</span>
          )}
        </div>
      </div>
      ...
      {state.isWinner && state.winAmount && (
        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-yellow-500 text-black px-2 py-1 rounded text-xs font-bold animate-bounce">
          +{state.winAmount}
        </div>
      )}
