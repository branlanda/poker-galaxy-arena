...
export const useTournaments = (filters: TournamentFilters = DEFAULT_TOURNAMENT_FILTERS) => {
  ...
  const fetchTournaments = async () => {
    try {
      ...
      // Process data to include registered player counts
      ...
      toast({
        title: t('errors.fetchFailed', 'Failed to fetch tournaments'),
      ...
      .on('postgres_changes', 
          ...
          (payload) => {
            if (payload.eventType === 'INSERT') {
              ...
              toast({
                title: t('tournaments.new', 'New Tournament'),
                description: t('tournaments.newTournamentAdded', '"{name}" has been added', { name: newTournament.name }),
              });
              ...
              const deletedTournamentId = (payload.old as Tournament).id;
              setTournaments(prev => 
